...
pub fn hash_string_with_salt(s: String) -> Result<String, Response> {
    ...
    let hash = argon2.hash_password(s.as_bytes(), &salt).map_err(|e| ResponseBuilder {
...
pub fn compare_hashed_strings(
    ...
    let argon2 = Argon2::default();
    let parsed_hash = PasswordHash::new(&hashed).map_err(|e| ResponseBuilder {
...
pub async fn find_user_in_db(conn: &DbConn, name: String) -> Result<Option<crate::models::User>, Response> {
...
pub async fn update_user_last_seen(conn: &DbConn, search_id: i32, time: chrono::DateTime<Utc>) -> Result<(), Response> {
    use crate::schema::users::dsl::*;
